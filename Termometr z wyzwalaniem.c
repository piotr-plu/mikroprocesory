...
int main(void)
{
	...
	OCR1A = 31250; //czas odświeżania
	TIMSK = (1<<TOIE0)|(1<<OCIE1B);
	sei();
	ADMUX = (1<<REFS1)|(1<<REFS0); //Vref=2,56V
	ADCSRA = (1<<ADEN)|(1<<ADPS2)|(1<<ADIE)|(1<<ADATE); //włączenie ADC i preskaler 16
	SFIOR = (1<<ADTS2)|(1<<ADTS0); // wyzwalanie porównanie B T1
	//ADCSRA |= (1<<ADSC);
    while (1) 
    {
		if(pokaz==1){
